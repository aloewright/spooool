...
// studio/apps/web/client/components/panels/FullBookPanel.tsx.
//
// Changes vs the studio source:
//   - UI primitives import from ../ui/* (the content-hub copies) and `api`/
//     `queryKeys` from ../../lib/api.
//   - The export-button bounce previously used the studio's gsap timeline hook
//     (components/animation/use-gsap-timeline). spooool does not ship gsap, so
...
// deploy. The mutation's error is rendered inline (the `startExport.error`
...
export default function FullBookPanel({ projectId }: { projectId: string }): ReactNode {
...
function ExportMotionButton({
  ...
  // Subtle scale pulse while an export is in flight (framer-motion replaces the
  // studio's gsap timeline). `asChild` lets the framer <motion.button> carry the
  // Button styling without nesting a second button.
  return (
    <Button asChild variant={variant} disabled={disabled}>
      <motion.button
        ...
        disabled={disabled}
        onClick={onClick}
        animate={pending ? { scale: [1, 0.98, 1] } : { scale: 1 }}
        transition={pending ? { duration: 0.32, repeat: Number.POSITIVE_INFINITY } : { duration: 0.2 }}
      >
        {icon}
...
function ReadinessRow({ label, ready }: { label: string; ready: boolean }): ReactNode {
  return (
...
function RenderJobsList({ jobs }: { jobs: RenderJob[] }): ReactNode {
  if (!jobs.length) {
