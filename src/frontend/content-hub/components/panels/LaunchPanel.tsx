// Go-to-market handoff — ported from
// studio/apps/web/client/components/panels/LaunchPanel.tsx.
//
// Changes vs the studio source:
//   - UI primitives import from ../ui/*; api/queryKeys/types from ../../lib/api.
//
// DEFERRED ACTION: "Generate brief" starts the gtm-brief workflow, which is
// guarded off until env.RENDER_WORKER / *_WORKFLOW bindings deploy. The
// mutation's `.error.message` renders inline so a failed generate surfaces the
// backend's "unavailable" message without crashing the panel; the brief query
// polls until a brief exists (it simply never arrives while deferred).
...
export default function LaunchPanel({ projectId }: { projectId: string }): ReactNode {
...
function LaunchBriefPreview({ brief }: { brief: GtmBrief }): ReactNode {
  return (
