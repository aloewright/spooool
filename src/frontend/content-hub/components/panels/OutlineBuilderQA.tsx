// Outline questionnaire wizard — ported from
// studio/apps/web/client/components/panels/OutlineBuilderQA.tsx.
//
// Changes vs the studio source:
//   - Wizard primitives import from ../wizard (spooool's content-hub copy of
//     the studio's components/studio/wizard) instead of ../studio/wizard.
//   - OUTLINE_FRAMEWORKS imports from ./outline-frameworks (the trimmed subset
//     of the studio panels/_shared that this wizard actually uses).
//   - On success it navigates to the spooool-absolute typed route
//     /studio/$projectId/outline (was /$projectId/outline in the studio's
//     own route tree). That route is registered in this PR.
...
import { ChoiceCard, FieldChip, Step } from '../wizard';
import { OUTLINE_FRAMEWORKS } from './outline-frameworks';
...
export default function OutlineBuilderQA({ project }: { project: Project }) {
  ...
  const generate = useMutation({
    ...
    onSuccess: async () => {
      ...
      navigate({ to: '/studio/$projectId/outline', params: { projectId: project.id } });
    },
  });
