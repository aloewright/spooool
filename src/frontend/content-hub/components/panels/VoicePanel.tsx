// Author-voice library + Post Pilot import — ported from
// studio/apps/web/client/components/panels/VoicePanel.tsx.
//
// Changes vs the studio source:
//   - UI primitives import from ../ui/*; api/queryKeys from ../../lib/api.
//   - The two helpers it needs (POSTPILOT_SUGGESTIONS, postPilotGuideLabel) come
//     from ./panels-shared (the trimmed PR-4 subset of the studio _shared.ts).
//
// All actions here (create voice, import Post Pilot, assign voice, add sample)
// hit live, non-deferred /api/v1/* endpoints — no workflow bindings involved.
// Each mutation renders its `.error.message` inline, so a failure surfaces
...
import { POSTPILOT_SUGGESTIONS, postPilotGuideLabel } from './panels-shared';

export default function VoicePanel({ project }: { project: Project }): ReactNode {
...
function VoiceProfile({ voice, totalWords }: { voice?: Voice; totalWords: number }): ReactNode {
  if (!voice) {
    return (
