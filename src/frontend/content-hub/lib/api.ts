// Ported from studio/apps/web/client/lib/api.ts for the spooool content-hub.
//
// PATCHED for spooool:
//   - The studio source resolved a runtime app base via `withBase()` /
//     detectAppBase (so the same build could serve at "/" on book-cook.com and
//     at "/studio" on spooool.com/studio). In spooool the content-hub is part
//     of the spooool SPA and always talks to the SAME ORIGIN, so the base is
//     hard-stripped to "" — every call hits `/api/v1/*` directly.
//   - The 401 redirect target was `/sign-in` (studio's auth page); spooool's
//     login route is `/login`, so unauthorized responses redirect there.
import { QueryClient } from '@tanstack/react-query';

// Same-origin base. The studio's withBase()/detectAppBase indirection is gone:
// spooool serves the hub and the /api/v1/* backend from one origin.
...
async function fetchJson<T>(path: string, init?: RequestInit): Promise<T> {
  ...
  if (res.status === 401) {
    if (
      ...
      !redirectingToLogin &&
      // spooool's login route is /login (was /sign-in in the studio source).
