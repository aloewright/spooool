// Blog post BlockNote editor (sub-project #4, PR-6) — ported from
// studio/apps/web/client/routes/_hub.blogs.$blogId.posts.$postId.tsx.
//
...
// imported ONLY here (and in BlogPostEditor / ScriptSceneEditor) — both of which
// are lazy()'d in router.tsx — so Vite code-splits the BlockNote CSS into the
// editor's async chunk. It never reaches the eager index/vendor bundle. The
// editor surface is additionally wrapped in `.bn-scope`, a containment boundary
// kept narrow so the BlockNote/Mantine theme can't bleed past the editor box.
//
// Ported verbatim aside from:
//   - file-route → plain component (params via props from router.tsx).
//   - `@/shared/blog-formats` → `../shared/blog-formats`.
...
function Editor({ blog, post }: { blog: BlogDetail; post: BlogPost }) {
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    (async () => {
      const blocks = await editor.tryParseMarkdownToBlocks(post.draft_md);
