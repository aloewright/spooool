...
export async function refreshMarketDataset(
  env: Pick<Env, "STUDIO_DB" | "R2" | "AI_GATEWAY_BASE_URL" | "AI_GATEWAY_TOKEN">,
  now = new Date(),
  ...
  const db = drizzle(env.STUDIO_DB);
  await db
...
export async function latestDatasetSnapshot(env: Pick<Env, "STUDIO_DB">) {
  const [latest] = await drizzle(env.STUDIO_DB)
    .select()
...
async function pruneOldSnapshots(env: Pick<Env, "STUDIO_DB" | "R2">, now: Date) {
  const cutoff = new Date(now.getTime() - RETENTION_DAYS * 24 * 60 * 60 * 1000);
  const oldRows = await drizzle(env.STUDIO_DB)
    .select()
    .from(dataset_snapshots)
  ...
  await drizzle(env.STUDIO_DB).delete(dataset_snapshots).where(lt(dataset_snapshots.created_at, cutoff));
}
