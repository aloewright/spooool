// ALO-143: thin client for the Loops (loops.so) email API. Two operations
// only — upsert a contact and send a lifecycle event — so the surface stays
// small and other modules don't grow direct fetch calls into Loops.
//
// Design:
...
// Upsert a contact by email. Loops' /contacts/update endpoint accepts a new
// or existing email and merges properties — perfect for a signup webhook
...
export async function upsertContact(
  ...
  return postJson('/contacts/update', env.LOOPS_API_KEY, contact);
}

// Mark a contact as unsubscribed. Used on account deletion so we don't
// keep mailing former users. We don't delete the contact — the user may
// rejoin and Loops needs the unsubscribed state for compliance.
export async function unsubscribeContact(
  ...
  return postJson('/contacts/update', env.LOOPS_API_KEY, {
    email,
    subscribed: false,
  });
}
...
export async function sendEvent(
  ...
  return postJson('/events/send', env.LOOPS_API_KEY, {
    email: args.email,
