...
async function processEarningsEvent(
  ...
  if (et === 'order.paid') {
    ...
    // Polar order objects carry a subscription_id field when the order was
    // generated by a subscription renewal. Extract it for the ledger row.
    const subscriptionId = typeof (data as Record<string, unknown>).subscription_id === 'string'
      ? (data as Record<string, unknown>).subscription_id as string
      : null;

    // creator_earnings.created_at is a TEXT column — write ISO 8601 so the
    // frontend's `new Date(iso)` parses correctly and ORDER BY sorts right.
      ...
      .prepare(
        `INSERT OR IGNORE INTO creator_earnings
           (id, user_id, kind, amount_cents, platform_fee_cents, currency,
            polar_order_id, polar_subscription_id, description, created_at)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      )
      .bind(
        crypto.randomUUID(),
  ...
  if (et === 'payout.created' || et === 'payout.paid') {
    ...
    // creator_payouts columns are TEXT — write ISO 8601 strings.
    const createdAtIso = new Date(nowMs).toISOString();
    const paidAt = et === 'payout.paid' ? createdAtIso : null;
