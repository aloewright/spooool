...
export function SideDrawer({
  ...
  const exportMutation = useMutation({
    ...
    onSuccess: () => navigate({ to: "/$projectId/book", params: { projectId } }),
  });

  return (
    <DrawerShell
      quickLinks={[
        { key: "home", label: "Home", icon: <Home className={iconCls} />, href: "/" },
        {
          key: "new",
          label: "New book",
      ...
      sections={[
        ...
        },
        {
          ...
          href: `/${projectId}/marketplace`,
        },
        {
          key: "voice",
          label: "Voice",
          icon: <Mic2 className={iconCls} />,
          href: `/${projectId}/voice`,
        },
        {
          key: "book",
          label: "Book",
    ...
    <DrawerShell
      ...
      sections={[
        ...
        },
        {
          ...
          href: `/blogs/${blogId}/structure`,
        },
      ]}
      activeKey={active}
      exportPending={false}
      // Blog publishing lives in the workspace's em_dash panel.
      onExport={() => navigate({ to: "/blogs/$blogId", params: { blogId } })}
    />
  );
}
    ...
    <DrawerShell
      ...
      sections={[
        ...
        },
        {
          ...
          label: "Structure",
          icon: <Network className={iconCls} />,
          href: `/scripts/${scriptId}/structure`,
        },
      ]}
      activeKey={active}
      exportPending={false}
      // Scripts have no export yet; land back on the script home.
      onExport={() => navigate({ to: "/scripts/$scriptId", params: { scriptId } })}
    />
  );
}
...
function sectionFromPathname(pathname: string, projectId: string): StudioSection {
  ...
  // Chapter editor (/$id/chapters/$chId) and the bare /$id (which redirects
  // to /canvas) both fall back to the outline section.
  return "outline";
}

function blogSectionFromPathname(pathname: string, blogId: string): BlogSection {
  const base = `/blogs/${blogId}`;
