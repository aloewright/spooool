...
describe("chapters", () => {
  it("generates outline chapters and autosaves a chapter draft", async () => {
    ...
    const unversionedDraft = await SELF.fetch(`http://localhost:5173/api/v1/chapters/${chapterId}`, {
      method: "PATCH",
      headers,
    ...
    const malformedDraft = await SELF.fetch(`http://localhost:5173/api/v1/chapters/${chapterId}`, {
      method: "PATCH",
      headers,
      body: JSON.stringify({
    ...
    const newerDraft = await SELF.fetch(`http://localhost:5173/api/v1/chapters/${chapterId}`, {
      method: "PATCH",
      headers,
      body: JSON.stringify({
    ...
    const staleDraft = await SELF.fetch(`http://localhost:5173/api/v1/chapters/${chapterId}`, {
      method: "PATCH",
      headers,
      body: JSON.stringify({
    ...
    const afterStale = await SELF.fetch(`http://localhost:5173/api/v1/chapters/${chapterId}`, { headers });
    // biome-ignore lint/suspicious/noExplicitAny: response shape from our own API
    ...
    const revisions = await SELF.fetch(`http://localhost:5173/api/v1/chapters/${chapterId}/revisions`, {
      headers,
    });
    ...
    const unauthorizedRevisions = await SELF.fetch(
      `http://localhost:5173/api/v1/chapters/${chapterId}/revisions`,
