...
describe("scripts", () => {
  it("create + list + get + plan + scenes + soft-delete + restore", async () => {
    ...
    const created = await SELF.fetch("http://localhost:5173/api/v1/scripts", {
      method: "POST",
      headers,
    ...
    const list = await SELF.fetch("http://localhost:5173/api/v1/scripts", { headers });
    // biome-ignore lint/suspicious/noExplicitAny: response shape from our own API
    ...
    const got = await SELF.fetch(`http://localhost:5173/api/v1/scripts/${id}`, { headers });
    ...
    const badStructure = await SELF.fetch(`http://localhost:5173/api/v1/scripts/${id}/plan`, {
      method: "POST",
      headers,
    ...
    const tooFew = await SELF.fetch(`http://localhost:5173/api/v1/scripts/${id}/plan`, {
      method: "POST",
      headers,
    ...
    const planned = await SELF.fetch(`http://localhost:5173/api/v1/scripts/${id}/plan`, {
      method: "POST",
      headers,
    ...
    const scenes = await SELF.fetch(`http://localhost:5173/api/v1/scripts/${id}/scenes`, {
      headers,
    });
    ...
    const replanTooFew = await SELF.fetch(`http://localhost:5173/api/v1/scripts/${id}/plan`, {
      method: "POST",
      headers,
    ...
    const unversionedDraft = await SELF.fetch(`http://localhost:5173/api/v1/scripts/${id}/scenes/${sceneId}`, {
      method: "PATCH",
      headers,
      body: JSON.stringify({ draft_md: "Missing its write order." }),
    });
    expect(unversionedDraft.status).toBe(400);
    const patchScene = await SELF.fetch(`http://localhost:5173/api/v1/scripts/${id}/scenes/${sceneId}`, {
      method: "PATCH",
      headers,
      body: JSON.stringify({
        title: "Storm Watch",
    ...
    const gotScene = await SELF.fetch(
      `http://localhost:5173/api/v1/scripts/${id}/scenes/${sceneId}`,
      {
        headers,
      },
    );
    ...
    const missingScene = await SELF.fetch(
      `http://localhost:5173/api/v1/scripts/${id}/scenes/${crypto.randomUUID()}`,
      {
        headers,
      },
    );
    ...
    await SELF.fetch(`http://localhost:5173/api/v1/scripts/${id}/scenes/${secondSceneId}`, {
      method: "PATCH",
      headers,
      body: JSON.stringify({
    ...
    const promoted = await SELF.fetch(
      `http://localhost:5173/api/v1/scripts/${id}/scenes/${secondSceneId}`,
      {
        headers,
      },
    );
    ...
    const staleDraft = await SELF.fetch(`http://localhost:5173/api/v1/scripts/${id}/scenes/${secondSceneId}`, {
      method: "PATCH",
      headers,
      body: JSON.stringify({
    ...
    const afterStale = await SELF.fetch(`http://localhost:5173/api/v1/scripts/${id}/scenes/${secondSceneId}`, {
      headers,
    });
    // biome-ignore lint/suspicious/noExplicitAny: response shape from our own API
    ...
    const del = await SELF.fetch(`http://localhost:5173/api/v1/scripts/${id}`, {
      method: "DELETE",
      headers,
    });
    expect(del.status).toBe(204);

    const list2 = await SELF.fetch("http://localhost:5173/api/v1/scripts", { headers });
    // biome-ignore lint/suspicious/noExplicitAny: response shape from our own API
    const items2 = (await list2.json()) as any;
    // biome-ignore lint/suspicious/noExplicitAny: row shape from our own API
    expect(items2.items.find((s: any) => s.id === id)).toBeFalsy();

    const deleted = await SELF.fetch("http://localhost:5173/api/v1/scripts/deleted/recent", {
      headers,
    });
    // biome-ignore lint/suspicious/noExplicitAny: response shape from our own API
    ...
    const restore = await SELF.fetch(`http://localhost:5173/api/v1/scripts/${id}/restore`, {
      method: "POST",
      headers,
    });
    expect(restore.status).toBe(200);

    const list3 = await SELF.fetch("http://localhost:5173/api/v1/scripts", { headers });
    // biome-ignore lint/suspicious/noExplicitAny: response shape from our own API
  ...
  it("preserves an explicit status racing first-draft promotion and returns current versions", async () => {
    ...
    expect(planned.status).toBe(200);
    const scenes = await SELF.fetch(`http://localhost:5173/api/v1/scripts/${id}/scenes`, { headers });
    // biome-ignore lint/suspicious/noExplicitAny: response shape from our own API
    const scenesBody = (await scenes.json()) as any;
    const sceneId = scenesBody.items[0].id;

    const [explicitStatus, firstDraft] = await Promise.all([
      SELF.fetch(`http://localhost:5173/api/v1/scripts/${id}/scenes/${sceneId}`, {
        method: "PATCH",
        headers,
        body: JSON.stringify({ status: "drafted" }),
      }),
    ...
    const afterRace = await SELF.fetch(`http://localhost:5173/api/v1/scripts/${id}/scenes/${sceneId}`, {
      headers,
    });
    // biome-ignore lint/suspicious/noExplicitAny: response shape from our own API
    ...
    const metadataPatch = await SELF.fetch(`http://localhost:5173/api/v1/scripts/${id}/scenes/${sceneId}`, {
      method: "PATCH",
      headers,
  ...
  it("rejects a logline that is too short", async () => {
    const cookie = await signUp();
    const headers = { "Content-Type": "application/json", cookie };
    const res = await SELF.fetch("http://localhost:5173/api/v1/scripts", {
      method: "POST",
      headers,
  ...
  it("enforces the short film planning threshold", async () => {
    ...
    const enough = await SELF.fetch(`http://localhost:5173/api/v1/scripts/${id}/plan`, {
      method: "POST",
      headers,
